'''
   Exercise 2: Investigating the signal topology.
   Performs full kinematic event selection and plots all key substructure
   variables for the b*->tW signal MC.
'''
import ROOT, sys, os
sys.path.append('./')
...
def select(setname, year):
    ...
    # ── 9. Remaining kinematic variables ────────────────────────────────────
    ...
    try:
        a.PrintNodeTree(plotdir + 'signal_tree_ex2.dot', verbose=True)
...
if __name__ == '__main__':
    ...
    for setname in signal_names:
        ...
        if infile is None:
            raise TypeError('rootfiles/exercise1.root not found - run with --select first')
    ...
    for varname in varnames.keys():
        if varname not in histgroups[signal_names[0]].keys():
            ...
            continue
        plot_filename = plotdir + 'exercise2_{}.png'.format(varname)
        sig = signal_names[0]
        EasyPlots(
            name      = plot_filename,
